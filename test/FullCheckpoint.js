...
describe("full checkpoint", () => {
  ...
  const fixture = async () => {
    ...
    const [block1, cblock1Encoded, block1Hash] = composeAndSignBlockSubnet(
      1,
      1,
      0,
      block0Hash,
    ...
    const customeBlock1 = { hash: block1Hash, encoded: cblock1Encoded };
  ...
  describe("test checkpoint real block data", () => {
    ...
    it("should verify roots", async () => {
      const block1Hash = blockToHash(block1Encoded);
      const roots = await checkpoint.getRoots(block1Hash);
